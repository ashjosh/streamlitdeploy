...
def _fetch_transcript_via_library(video_id: str) -> str:
    """Try youtube-transcript-api across old/new package interfaces."""
    ...
    # Interface 1 (older): classmethod get_transcript(...)
    if hasattr(api_cls, "get_transcript"):
        transcript_rows = api_cls.get_transcript(video_id, languages=["en", "en-US", "en-GB", "hi"])

    # Interface 2 (newer): instance().fetch(video_id, languages=[...])
    if transcript_rows is None:
        api_instance = api_cls()
        if hasattr(api_instance, "fetch"):
            transcript_rows = api_instance.fetch(video_id, languages=["en", "en-US", "en-GB", "hi"])

    if transcript_rows is None:
        raise ValueError("youtube-transcript-api detected but no compatible transcript method found.")

    parts = [item.get("text", "").strip() for item in transcript_rows if isinstance(item, dict) and item.get("text")]
...
def _fetch_transcript_via_timedtext(video_id: str) -> str:
    """Try YouTube timedtext endpoint in both JSON3 and XML modes."""
    candidates = [
        ...
        f"https://www.youtube.com/api/timedtext?lang=hi&v={video_id}",
        f"https://www.youtube.com/api/timedtext?lang=hi&kind=asr&v={video_id}",
    ]
    ...
    for url in candidates:
        ...
        # XML mode
        if "<transcript" in text_body or text_body.startswith("<?xml"):
            try:
                root = ET.fromstring(text_body)
                parts = [(node.text or "").strip() for node in root.findall(".//text")]
                parts = [p for p in parts if p]
                if parts:
                    return " ".join(parts)
...
def _fetch_transcript_via_page_scrape(video_id: str) -> str:
    ...
    if body.startswith("{"):
        try:
            ...
            events = []
    elif "<transcript" in body or body.startswith("<?xml"):
        try:
